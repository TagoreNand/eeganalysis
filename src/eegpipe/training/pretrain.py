...
def main(cfg: DictConfig) -> None:
    ...
    rec = bundle.groups
    log.info(
        "Pretraining encoder on %d unlabelled epochs from %d recordings",
        len(rec),
        len(set(rec.tolist())),
    )
    ...
    ds = RelativePositioningDataset(
        X,
        rec,
        tau_pos=cfg.training.tau_pos,
        tau_neg=cfg.training.tau_neg,
    ...
    encoder = EpochEncoder(
        n_channels=X.shape[1], emb_dim=cfg.model.emb_dim, sfreq=cfg.model.get("sfreq", 100)
    )
