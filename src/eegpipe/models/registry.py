...
def build_model(cfg, n_channels: int, n_times: int, n_classes: int,
                class_weights=None) -> LitEEGClassifier:
    ...
    backbone = BACKBONES[name](n_channels=n_channels, n_times=n_times, n_classes=n_classes, **params)
...
def load_pretrained_encoder(backbone, ckpt_path: str) -> None:
    ...
    log.info("Loaded pretrained encoder from %s (missing=%d unexpected=%d)",
             ckpt_path, len(missing), len(unexpected))


def build_sequence_model(cfg, n_channels: int, n_times: int, n_classes: int,
                         class_weights=None, class_names=None) -> LitSequenceClassifier:
    ...
    if enc_kind == "foundation":
        ...
        encoder = build_foundation_encoder(
            foundation_name, in_channels=n_channels,
            emb_dim=params.get("emb_dim", 128), freeze=freeze)
    ...
    return LitSequenceClassifier(backbone, n_classes=n_classes, class_weights=class_weights,
                                 class_names=class_names, **wrap_kw)
